...
public final class SpecificationDependencyImpl extends AbstractDependencyImpl<Set<SpecificationVersionDescription>> implements SpecificationDependency {
    ...
    @Override
    public boolean isCompatibleWith(final SpecificationDependency otherSpecificationDependency) {
        if (otherSpecificationDependency == this || otherSpecificationDependency.equals(this)) {
            return true;
        }

        for (final SpecificationVersionDescription specificationVersionDescription : getDependencyValue()) {
            for (final SpecificationVersionDescription otherSpecificationVersionDescription : otherSpecificationDependency.getDependencyValue()) {
                if (specificationVersionDescription.isCompatibleWith(otherSpecificationVersionDescription)) {
                    return true;
                }
            }
        }
